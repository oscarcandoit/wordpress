---
url: https://woocommerce.com/documentation/products/extensions/flexi-custom-order-status
scraped_at: 2025-10-20T02:30:57.115Z
---

[Skip to navigation](https://woocommerce.com/documentation/products/extensions/flexi-custom-order-status/#main-navigation) [Skip to content](https://woocommerce.com/documentation/products/extensions/flexi-custom-order-status/#page)
...
This article addresses some of the frequently asked questions about using the Flexi Custom Order Status for WooCommerce plugin. Can I send custom emails for custom order status changes? Yes, custom emails can be setup in WooCommerce->Settings->Custom Order Status->Emails. Can I set a different email for each custom order status? Yes, you can set a \[…\]

- [**Modify WooCommerce Order Statuses to custom order statuses**](https://woocommerce.com/document/flexi-custom-order-status/modify-woocommerce-order-statuses-to-custom-order-statuses/)
Custom Order Status for WooCommerce plugin has introduced a new tab that will easily allow you to modify standard WooCommerce statuses. Here’s how to go about it: Go to WooCommerce > Settings > Custom Order Status >WooCommerce Order Status tab as shown below: Here you find all the default WooCommerce Order Statuses. The list is as follows: When \[…\]

- [**Notify Admins via Emails about custom status change after a period of time**](https://woocommerce.com/document/flexi-custom-order-status/notify-admins-via-emails-about-custom-status-change-after-a-period-of-time/)
Custom Order Status For WooCommerce plugin’s new feature will let you send notification emails if orders have been in a particular status for more than some time. To configure the settings, go to WooCommerce > Settings > Custom Order Status > Notify Emails. Here’s how the page will appear: Emails: If you wish to make use of this setting, \[…\]

- [**Notify customers about change in order status with SMS**](https://woocommerce.com/document/flexi-custom-order-status/notify-customers-about-change-in-order-status-with-sms/)
Notifying customers about changes in their custom order status is a great way to keep them engaged and updated about their orders. With Custom Order Status for WooCommerce plugin, admins can now notify customers via SMS about the change in their custom order status. Here’s how to go about it: Go to WooCommerce > Settings > Custom Order Status \[…\]

- [**Sending Email Notifications**](https://woocommerce.com/document/flexi-custom-order-status/sending-email-notifications/)
Once the custom statuses have been created, you can set up emails that will be triggered when the order status changes to custom status. Email notification is sent to the customer and/or admin in order to confirm the order status change to them. Email notifications can be set in two ways- 1) at the Global level  2) at the Custom \[…\]

- [**Set Custom Order Status Rules**](https://woocommerce.com/document/flexi-custom-order-status/set-custom-order-status-rules/)
The Custom Order Status Rule feature allows you to automate the transition of order statuses based on predefined conditions. This feature is handy for managing orders efficiently by specifying rules that dictate how and when an order status should change. Add Order Status Rule To add the custom order status rule, follow the path WooCommerce -> \[…\]
